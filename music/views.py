...
def top_artists():
  ...
  headers = {
    "X-RapidAPI-Key": "324a455c66mshbd052b12b80e38bp15c602jsn3b0ae1b5d832",
...
def get_audio_details(query):
  ...
  if response.status_code ==200:
    ...
    if 'youtubeVideo' in response_data and 'audio' in response_data['youtubeVideo']:
      ...
      if audio_list:
        first_audio_url = audio_list[0].get("url")
        duration_text = audio_list[0].get("durationText")
...
def music(request,pk):
  ...
  if response.status_code ==200:
    ...
    audio_url = audio_details[0] if audio_details is not None else "No URL"

    duration_text = audio_details[1] if audio_details is not None else "No durationtext"

    if get_track_image( track_id, track_name ):
      trackimage = get_track_image( track_id, track_name )
    else:
...
def index(request):
  ...
  first_top_ten_artist = artists_info[:10]
  ...
  context = {
    'top_ten_artist' : first_top_ten_artist,
...
def profile(request,pk):
  ...
  if response.status_code == 200:
    data = response.json()
    name=data["name"]
    montly_listeners = data["stats"]["monthlyListeners"]
    ...
    artists_data = {
      "name": name,
      "monthlyListeners": montly_listeners,
      "header_url": header_url,
      "topTracks": top_tracks
    }
  else:
    artists_data ={}
  return render(request,'profile.html', artists_data)
